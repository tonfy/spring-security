 ...
 * query param the page will automatically submit its form with the token value.
 *
 * @author Marcus da Coregio
 * @since 6.4
 */
public class DefaultOneTimeTokenSubmitPageGeneratingFilter extends OncePerRequestFilter {
	...
	private String generateHtml(HttpServletRequest request) {
		...
		String inputValue = autoSubmitForm ? ("value=\"" + tokens.get(0) + "\"") : "";
		String input = "<input type=\"text\" id=\"token\" name=\"token\" " + inputValue
		...
		return """
				...
				+ "<form class=\"login-form\" action=\"" + this.loginProcessingUrl + "\" method=\"post\">" + """
						...
						"""
				+ (autoSubmitForm ? "<script>window.onload = function() { document.forms[0].submit(); }</script>" : "")
				+ """
						</body>
						</html>
						""";
